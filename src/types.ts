...
export type QueryParams = QueryByCityId &
  QueryByCityName &
  QueryByGeoCoordinates &
  QueryByZipCode &
  WithAppId &
  WithLangModeUnits &
...
export type WithLangModeUnits = WithLang & WithMode & WithUnits
...
export type OneCallData = ByGeoGraphicCoordinates & {
  exclude?: ('current' | 'minutely' | 'hourly' | 'daily' | 'alerts')[]
} & Partial<WithAppId & WithV & WithLangModeUnits>

export interface OneCallResult {
  current: OneCallResultCurrent
  daily: OneCallResultDaily[]
  hourly: OneCallResultHourly[]
  lat: Latitude
  lon: Longitude
  minutely: OneCallResultMinutely[]
  summary?: string
  timezone: string
...
export type FiveDay3HourForecastData = (
  ...
  Partial<WithAppId & WithV & WithLangModeUnits> & { cnt?: number }
...
export type AirPollutionCurrentData = ByGeoGraphicCoordinates &
  Partial<WithAppId & WithV>

export type AirPollutionForecastData = ByGeoGraphicCoordinates &
  Partial<WithAppId & WithV>

export type AirPollutionHistoricalData = ByGeoGraphicCoordinates &
  Partial<WithAppId & WithV> & { start: number; end: number }
